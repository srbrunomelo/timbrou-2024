...
import type { THTTPRequest, THTTPRequestOptions, THTTPResponse } from '../types';
...
function FetchHTTPRequestSingleton(): THTTPRequest {
  ...
  async function requestWithBody(
    ...
    console.log(`Time taken for ${method} ${formattedUrl} is ${endTime - startTime} ms`);
  ...
  async function get<T>(url: string, options: THTTPRequestOptions): Promise<THTTPResponse<T>> {
    const formattedUrl = formatUrl(url, options?.params);
    const startTime = performance.now();
    const response = await fetch(formattedUrl, {
      ...options,
    ...
    const data = await response.json();
    const endTime = performance.now();
    console.log(`Time taken for get ${formattedUrl} is ${endTime - startTime} ms`);
    return {
      data,
      status: response.status,
    };
  ...
  async function remove<T>(url: string, options: THTTPRequestOptions): Promise<THTTPResponse<T>> {
    const formattedUrl = formatUrl(url, options?.params);
